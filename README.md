...
| `discord`                    | `{ botToken: string }` \| `(discordUserId, message) => Promise<void>` | No       | —                                       | Configures 2FA delivery via Discord DM. Pass `{ botToken }` to use the built-in `discord.js`-backed sender (logs in once at `createAuthRouter` construction time; the bot must share a guild with each target user, or the user must allow DMs from server members), or a callback to deliver the message yourself.                                                                                                                         |
...
| `DiscordSender`       | Persistent `discord.js` client wrapper; used to build the default `discord` sender. |
