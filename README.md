...
- **Built-in 2FA delivery** — email (via [Resend](https://resend.com)) and Discord DM (via [`kai-discord-sender`](https://github.com/Holayn/discord-messenger-bot)), with a `development` mode that logs codes to the console instead of sending them
...
const { apiRouter, pageRouter, requireAuth, userStore } = createAuthRouter({
  ...
  // unless `development` is set — see the Options table below. Each can instead
  // be a callback — (to, subject, body) for email, (discordUserId, message) for
  // discord — if you want to route delivery through your own infra (e.g. an
  // internal notification microservice) instead of kaiauth's built-in senders.
  email: {
...
`POST /auth` and `POST /auth/2fa` accept an optional `redirect` field and, on success, return `{ success: true, redirectTo }`. The server — not the client — decides `redirectTo`: if `redirect` is a safe same-origin path (starts with a single `/`, not `//`), it's echoed back as-is; otherwise the configured `defaultRedirect` (default `'/'`) is used. A client should treat `redirectTo` as the only source of truth and navigate there (`location.href = data.redirectTo`) rather than re-deriving or re-validating it — this is exactly what the built-in login page does with the `?redirect=` query param it was reached with.

Keeping this validation server-side (rather than duplicating it in client-side JS) means there's one implementation to test and audit instead of two that can drift out of sync, and it leaves room to make the decision smarter later (e.g. a per-role landing page) since it runs in a request-scoped handler that already has the authenticated user, rather than being baked into a static page render.
...
| `email`                      | `{ apiKey, from }` \| `(to, subject, body) => Promise<void>`          | No       | —                                       | Configures 2FA delivery via email. Pass `{ apiKey, from }` to use the built-in [Resend](https://resend.com)-backed sender, or a callback to deliver the message yourself (e.g. via an internal notification service) — kaiauth still decides the subject/body wording, your callback just has to send it.                                                                                                                                   |
| `discord`                    | `{ botToken: string }` \| `(discordUserId, message) => Promise<void>` | No       | —                                       | Configures 2FA delivery via Discord DM. Pass `{ botToken }` to use the built-in [`kai-discord-sender`](https://github.com/Holayn/discord-messenger-bot)-backed sender (logs in once at `createAuthRouter` construction time; the bot must share a guild with each target user, or the user must allow DMs from server members), or a callback to deliver the message yourself.                                                                                                                         |
...
| `defaultRedirect`            | `string`                                                              | No       | `'/'`                                   | Where a successful login lands when the request's `redirect` field is absent or isn't a safe same-origin path. Returned as `redirectTo` in the `POST /auth` / `POST /auth/2fa` success response — see [Post-Login Redirect](#post-login-redirect). Must be a same-origin path starting with a single `/`; `createAuthRouter` throws at construction time otherwise.                                                                         |
...
| ------ | ------------------------------- | ---- | -------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/auth`                         | No   |          | `{ username, password, redirect? }` | Login — validates credentials. With 2FA enabled, returns `{ twoFA: true, channel, emailFallbackAvailable }` if a code challenge is required (`channel` is `'discord'`, `'email'`, or `'development'` — delivering the code per [2FA Code Delivery](#2fa-code-delivery), or `500` if delivery fails); otherwise creates a session directly and returns `{ success: true, redirectTo }` — see [Post-Login Redirect](#post-login-redirect). |
| POST   | `/auth/2fa`                     | No   | Yes      | `{ twoFACode, redirect? }`          | Verify 2FA code and create session. Returns `{ success: true, redirectTo }` on success — see [Post-Login Redirect](#post-login-redirect).                                                                                                                                                                                                                                                                                                |
...
| `EmailSender`         | Resend-backed wrapper; used to build the default `email` sender.                    |
