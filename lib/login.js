 ...
 * @property {boolean} [enable2fa=true]
 *   When false, successful credential validation immediately returns
 *   `Status.SUCCESS` with no 2FA challenge or bypass-token handling.
 * @property {(token: string) => { username: string } | null} [getBypassToken]
 *   Look up existing 2FA bypass token; return entry or null.
 *   Required when `enable2fa` is true.
 ...
 * @property {number} [codeTtlMs]             – 2FA code lifetime, default 60 s (only used when enable2fa is true)
...
class LoginService {
   ...
   *
   * When `enable2fa` is true, a successful login returns `Status.TWO_FA_REQUIRED`
   * (or `Status.BYPASSED` if a valid bypass token is present).  When `enable2fa`
   * is false, a successful login returns `Status.SUCCESS` immediately.
   *
   * @param {string}      username
   * @param {string}      password
   * @param {string|null} existingBypassToken – current bypass cookie value, if any (ignored when enable2fa is false)
